...
def test_get_states_returns_list(db_session):
    db_session.query().order_by().offset().limit().all.return_value = [
        State(id=1, name="Active", sort_order=1)
    ]
...
def test_create_state_duplicate_name(db_session):
    db_session.query().filter().first.return_value = State(
        id=2, name="Inactive", sort_order=2
    )
    state_data = StateCreate(name="Inactive", description="Not active", sort_order=2)
